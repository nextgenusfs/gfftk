...
def paf2dict(paf, fasta, annotation={}, min_mapq=2):
    ...
    with zopen(paf) as infile:
        for line in infile:
            ...
            if not cs:
                sys.stderr.write(
                    "Error: {} file does not contain cs:Z: tags, run minimap2 with the -x splice --cs flags\n".format(
                        paf
                    )
                )
                return {}
            ref_coords, query_coords, properSplice = cs2coords(
                int(tstart), qstart, int(qlen), strand, cs
            )
